      ...
      {isLogin ? (
        ...
        <form className="space-y-4" onSubmit={onRegister}>
          <TextField
            ...
            name="fullName"
            onChange={(value) =>
              setRegisterForm({ ...registerForm, fullName: value })
            }
            required
          ...
          <fieldset>
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              {roleOptions.map((option) => {
                const checked = registerForm.role === option.value;

                return (
                  <label
                    key={option.value}
                    className={`min-h-[96px] cursor-pointer rounded-md border p-3 transition ${checked
    ...
    <div>
      ...
      <span className="relative block">
        <input
          ...
          className={`h-11 w-full rounded-md border border-slate-300 bg-white px-3 text-sm text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100 ${isPassword ? "pr-11" : ""
            }`}
        />
        {isPassword ? (
          <button
...
function NoticeBanner({ notice }: { notice: Exclude<Notice, null> }) {
  return (
    <div
      className={`mt-4 rounded-md border px-4 py-3 text-sm ${notice.type === "success"
...
function tabClassName(active: boolean): string {
  return `h-10 rounded-md px-3 text-sm font-semibold transition ${active
